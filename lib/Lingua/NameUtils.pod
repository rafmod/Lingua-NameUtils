...
export tags are available for the C<use> statement:

=over 4

=item C<:all> - All functions

=item C<:case> - I<namecase>, I<gnamecase>, I<fnamecase>, I<namecase_exception>, I<normalize>

=item C<:split> - I<namesplit>, I<nameparts>, I<namesplit_exception>, I<normalize>
...
=item B<S<$name = namecase([$name[, $part[, $given_names]]])>>

Returns the supplied name with the capitalization fixed. See the B<EXAMPLES>
...
If not supplied, the C<$name> argument defaults to C<$_>.
...
For a given name or names by itself (same as I<gnamecase()>, see below):

 $given_names = namecase("JOHN PETER", 'given');

For a family name by itself (same as I<fnamecase()>, see below):
...
I<fnamecase()>, see below):

 $family_name = namecase("SMITH", 'family', "JOHN PETER");

=item B<S<$given_names = gnamecase([$given_names])>>

Returns the supplied given name(s) with the capitalization fixed. Same as:
C<S<namecase($given_names, 'given')>>. Given names aren't capitalized in
exactly the same way as family names.

If not supplied, the C<$given_names> argument defaults to C<$_>.

=item B<S<$family_name = fnamecase([$family_name[, $given_names]])>>

Returns the supplied family name with the capitalization fixed. Same as:
C<S<namecase($family_name, 'family'[, $given_names])>>.

If not supplied, the C<$family_name> argument defaults to C<$_>.

The C<$given_names> argument is technically optional, but it should be
...
need for such individual case exceptions, the C<$given_names> argument will
become necessary everywhere, so it's best to supply it from the start.

=item B<S<namecase_exception($bespoke_capitalized_name, ...)>>
...
form: I<S<"Family_name, Given_names">>, capitalized as specified by the
named person:
...
=item B<S<$full_name = namesplit([$full_name])>>

Returns the supplied full name converted to the unambiguous form:
I<S<"Family_name, Given_names">> with the capitalization fixed.

If not supplied, the C<$full_name> argument defaults to C<$_>.

The C<$full_name> argument is expected to be in either form:
I<S<"Given_names Family_name">> or I<S<"Family_name, Given_names">>. Note
...
Latin script are identified. See the B<EXAMPLES> section below. But
...
several countries. It also recognizes the 190 most common Korean names (98%
of the population) in Hangul, Hanja, and romanized. It also recognizes the
209 Vietnamese names (100% of the population, apparently). There are too
many Japanese family names (over 300,000) to maintain a list of them, so
this module delegates to I<Lingua::JA::Name::Splitter> which employs a
statistical method to identify Japanese family names written in Kanji and
Kana. I don't know what proportion of Japanese family names it identifies.

=item B<S<($family_name, $given_names) = nameparts([$full_name])>>
...
The C<$full_name> argument is expected to be in either form:
I<S<"Given_names Family_name">> or I<S<"Family_name, Given_names">>.

This function converts the corresponding return value of I<namesplit()> into
a two-item array. See I<namesplit()> above for more details. If the name
...
=item B<S<namesplit_exception($full_name_in_comma_form, ...)>>
...
automatically identified, such as I<S<"Ah Mu, Corie">>, and even complex
given names that would be misrecognized, such as I<S<"de Sousa, Fatima de Gois">>.
...
single space, any space before a comma-like character or hyphen-like
character removed, and with a space added after any comma-like character, if
one is not already present there.

If not supplied, the C<$name> argument defaults to C<$_>.

=item B<S<normalize($func)>>
...
normalization. A likely choice would be I<Unicode::Normalize::NFD>.
...
=item B<$kcname = kc($name)> [Internal]

This function is internal to the module and is never exported. It returns
hash keys created from names for looking up internal data. It assumes that
the supplied name is defined. It assumes that I<nametrim()> has already
processed the supplied name. It's like I<fc()> (or I<lc()> on I<S<perl
v5.14>>) except that it also replaces non-ASCII apostrophe-like characters
with the ASCII apostrophe character, and it replaces non-ASCII hyphen-like
characters with the ASCII hyphen-minus character.

It is documented here so as to satisfy I<Pod::Coverage> which thinks this
should be here. It would only be useful externally if the application had a
hash keyed by people's names.

=back

=head1 EXAMPLES

These examples show the default I<namecase()> output for various forms of
names. They also show which name forms are automatically recognized by
I<namesplit()>. Note that non-ASCII letters and punctuation in these
examples have been replaced with the closest ASCII equivalents to avoid
problems with some implementations of I<*roff>. I<namesplit()> also supports
...
their names cased incorrectly (e.g., S<Da Vinci> rather than S<da Vinci>),
...
"word". For example, a Greek family name can start with I<el>, but a Spanish family
name can start with I<El>. This module favours the most likely case (i.e.,
Spanish in this example). For other cases, this can be corrected with case
exceptions.

Similarly, a Hebrew patronymic name can start with I<ben>, but an Anglo
given name can be I<Ben>. That's fine if I<fnamecase()> or I<gnamecase()>
are used, supplied with just the given name(s) or the family name,
respectively, but I<namecase()> supplied with an ambiguous full name will
...
ambiguous I<ben> to be correctly identified as a patronymic prefix.

Similarly, when it comes to splitting/identifying the given names and the
family name within a full name with I<namesplit()> or I<nameparts()>, the
word I<ben> is not interpreted as the start of a patronymic name (in the
absence of other clues as indicated above), because I<Ben> is more likely to
be an Anglo middle name (although I<bat> is always interpreted as the start
...
start or the end of a full name. This module detects them in either format.
But there can be false positives when a given name looks the same as a
romanized CKV family name. For example, I<namesplit()> works better for
...
cases might arise due to not knowing which language a romanized name is
...
I<Unicode::Normalize::NFD> (or maybe even I<Unicode::Stringprep>) before
...
internal data (with I<normalize()>) to match your application's choice of
normalization.

Note: This module does also work with strings in non-utf8 source code. It
does not require utf8 source code. But it does require I<S<perl v5.14>> or
later.

=head1 BUGS

The I<nameparts()> function probably should have been called I<namesplit()>,
because it returns an array, and the I<namesplit()> function probably should
...
needed, two individual case exceptions, and about a thousand split
exceptions.
...
L<Lingua::EN::TitleParse>,
L<Lingua::EN::NameCase>,
L<Lingua::EN::NameParse>,
L<Lingua::JA::Name::Splitter>,
L<String::ProperCase::Surname>,
L<Unicode::Normalize::NFD>,
L<Unicode::Stringprep>.
